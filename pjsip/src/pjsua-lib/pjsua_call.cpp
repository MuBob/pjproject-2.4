 ...
 * The INFO method.
 */
static char  remote_id[50];

acc_intfo_t  acc_info_local;
...
#ifndef MAX_CPK_ID_LEN
#define MAX_CPK_ID_LEN 50
#endif
static char remote_id_buf[MAX_CPK_ID_LEN + 1];
static char* parse_remoteid(char* uri)
{
	char* start;
	char* end;
	int len;
	...
	//if (!end) return NULL;
	len = end - start - 1;
	strcpy(remote_id_buf, start + 1);
	//remote_id_buf[len] = 0;
	return remote_id_buf;
}
char * get_ids()
{
...
void set_remote_id(char *cpk_id,int len){
	strcpy(remote_id, cpk_id);
}
/*
					 ...
					 pjsua_call_id *p_call_id)
{
	...
	char* cpk_id;
	strcpy(acc_info_local.remote_id , dest_uri->ptr);
	strcpy(acc_info_local.local_id, acc->cfg.id.ptr);
	cpk_id = parse_remoteid(dest_uri->ptr);
	if (cpk_id == NULL)
	{
		pjsua_perror(THIS_FILE, "Error Set Remote CPK ID", 0);
		goto on_error;
	}
	int len = strlen(cpk_id);
	set_remote_id(cpk_id, len);
    /* Attach user data */
...
#define URL_SIZE  64
	char * cpk_id;
	char from_buf[URL_SIZE];
	int len = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, rdata->msg_info.from->uri, from_buf, URL_SIZE-1);
	if (len >= 1)
		from_buf[len] = 0;

	strcpy(acc_info_local.remote_id, from_buf);
	strcpy(acc_info_local.local_id, pjsua_var.acc[acc_id].cfg.id.ptr);
