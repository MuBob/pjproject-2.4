...
void  on_pager_wrapper1(pj_str_t *from, pj_str_t* to, pj_str_t *body, pj_str_t* mimetype){
	...
	pj_str_t true_from = pj_str(acc_info_local.remote_id);
	pj_str_t true_to = pj_str(acc_info_local.local_id);
	
	if (pjsua_var.ua_cfg.cb.on_pager) {
		(*pjsua_var.ua_cfg.cb.on_pager)(0, &true_from, &true_to, NULL,
			mimetype, body);
	}
}
/*
 ...
 * route set and authentication.  mime type 可为空msg_data可谓空
 */
				   ...
				   void *user_data)
{
    ...
    pjsua_acc *acc;
    pj_status_t status;
	/*int len= strlen("中华人民共和国sldkfjasldkfj");
	strcpy(content->ptr , "中华人民共和国sldkfjasldkfj");
	...
	*/
	...
	//m_pSteg.SLock.lock();
	//m_pSteg.Send((void *)content->ptr, content->slen, 1);
	//m_pSteg.SLock.unlock();
