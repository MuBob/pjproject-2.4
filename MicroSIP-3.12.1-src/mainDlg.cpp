...
// static void ui_send_secret_message()
// {
// 	int length = 0;
// 	char *pSrc;
// 
...
LRESULT CmainDlg::onCallState(WPARAM wParam, LPARAM lParam)
{
	...
	if (call_info->state == PJSIP_INV_STATE_CONFIRMED) {
		...
		connected_ornot = TRUE;
		con_dlg.Enable(TRUE);//air
		if (!callTimer) {
			OnTimerCall(true);
	...
	if (call_info->state == PJSIP_INV_STATE_DISCONNECTED) {
		connected_ornot = FALSE;
		con_dlg.Enable(FALSE);//air
	...
	: CBaseDialog(CmainDlg::IDD, pParent)
{
	...
	CString audioCodecsCaptions = _T("opus/48000/2;Opus 16 kHz;\
...
BOOL CmainDlg::OnInitDialog()
{
	...
	RegisterHotKey(AfxGetMainWnd()->m_hWnd, 111, MOD_ALT | MOD_CONTROL /*| MOD_NOREPEAT*/, 0x41);// air 2016.12.1
	con_dlg.Create(IDD_DIALOG_CON, this);
...
LRESULT CmainDlg::OnHotKey(WPARAM wParam, LPARAM lParam)
{
	...
	if (connected_ornot == TRUE) {
		switch (wParam)
		{
		case 111:
		{
//			con_dlg.modal_ornot = TRUE;
			con_dlg.ShowWindow(!con_dlg.IsWindowVisible());
			con_dlg.GetDlgItem(IDC_EDIT_MSG)->SetFocus();
			//con_dlg.DoModal();
			//con_dlg.modal_ornot = FALSE;
		}
		break;
		default:
			break;
...
void CmainDlg::MainPopupMenu()
{
	...
	CString str;
	tracker->GetMenuString(ID_DONATE, str, MF_BYCOMMAND);
	str.Append(_T(" $1"));
	MENUITEMINFO menuItemInfo;
	menuItemInfo.cbSize = sizeof(MENUITEMINFO);
	menuItemInfo.fMask = MIIM_STRING;
	menuItemInfo.dwTypeData = str.GetBuffer();
	tracker->SetMenuItemInfo(ID_DONATE, &menuItemInfo);
...
#endif
			SetSoundDevice(audio_output, true);
		}
		else {
...
LRESULT CmainDlg::onDialNumber(WPARAM wParam, LPARAM lParam)
{
