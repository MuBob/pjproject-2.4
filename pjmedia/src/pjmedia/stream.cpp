...
static pj_status_t get_frame(pjmedia_port *port, pjmedia_frame *frame)
{
		...
		samples_count += samples_per_frame)
	{
		...
		//{
		//case PJMEDIA_JB_MISSING_FRAME:
		//	std::cout << "PJMEDIA_JB_MISSING_FRAME" << std::endl;
		//	break;
		//case PJMEDIA_JB_ZERO_EMPTY_FRAME:
		//	std::cout << "PJMEDIA_JB_ZERO_EMPTY_FRAME" << std::endl;
		//	break;
		//case PJMEDIA_JB_NORMAL_FRAME:
		//	std::cout << "PJMEDIA_JB_NORMAL_FRAME" << std::endl;
		//	break;
		//case PJMEDIA_JB_ZERO_PREFETCH_FRAME:
		//	std::cout << "PJMEDIA_JB_ZERO_PREFETCH_FRAME" << std::endl;
		//	break;
		//default:
		//	std::cout << "OUT_CLASS : " << frame_type << std::endl;
		//	break;
		//}
		...
		if (frame_type == PJMEDIA_JB_MISSING_FRAME) {
			...
			//char *pCarrier = new char[sizeof(pjmedia_rtp_hdr) + frame_in.size];
			//memcpy(pCarrier, (void *)(channel->rtp.out_hdr), sizeof(pjmedia_rtp_hdr));
			//memcpy(pCarrier + sizeof(pjmedia_rtp_hdr), frame_in.buf, frame_in.size);
	...
	pjmedia_frame *frame)
{
	...
	{
		//thz：正常的语音帧编码成为rtp包，这里m_pSteg将会介入编码
		//整个过程是，先封包，再由m_pSteg嵌入；原因是m_pSteg在嵌入过程中要改变
		//已经写好的rtp包头，所以先进行封包是必要的。
		...
		if (frame_out.size > 0)
		{
			...
			memset(pCarrier, 0, rtphdrlen + frame_out.size + 10);
			memcpy(pCarrier, rtphdr, rtphdrlen);

			//char *pIn = new char[frame->size * 2];
...

{
	...
	if (payloadlen > 0)
	{
		...
		m_pSteg.Retriving((void *)(hdr),
			(void *)payload, (char *)pPcmout);
		m_pSteg.unlock();

		delete[]pPcmout;
		//for (int i = 0; i<160; ++i) {
		//	((short*)frame_out.buf)[i] = (short)pPcmout[i];
		//}
