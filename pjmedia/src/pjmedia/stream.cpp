				  ...
				  pjmedia_frame *frame )
{
	...
	status = pjmedia_codec_encode( stream->codec, frame,
				       ...
				       sizeof(pjmedia_rtp_hdr),
				       &frame_out);
//	PJ_LOG(4, (THIS_FILE, "after Encode out frame size = %d", frame_out.size));
	if (status != PJ_SUCCESS) {
	    LOGERR_((stream->port.info.name.ptr,
		    "Codec encode() error", status));
	    return status;
	...
	status = pjmedia_rtp_encode_rtp( &channel->rtp,
		...
		if (frame_out.size > 0)
		{
			...
			if (channel->pt == PJMEDIA_RTP_PT_ILBC)
			{
...
//				PJ_LOG(4, (THIS_FILE, "Encode channel iLBC"));
				m_pSteg.lock();
...
//				PJ_LOG(4, (THIS_FILE, "Encode channel PCMU/PCMA"));
				m_pSteg.lock();
...

{
	...
	*p_stream = stream;
	...
	mode20_30 = 20;
	m_pSteg.Create(pool);
	m_pSteg.Allocate(info->fmt.pt);
	m_pSteg.Control(info->fmt.pt, 0);
