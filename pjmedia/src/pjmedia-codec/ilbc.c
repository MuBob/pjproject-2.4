...
#include <pj/os.h>
	...
	#include "../../third_party/ilbc/iLBC.h"
#endif
				     ...
				     struct pjmedia_frame *output)
{
	...
	iLBCEncode((unsigned char *)output->buf + output->size, 
		ilbc_codec->enc_block, &ilbc_codec->enc, 0, NULL);
	//for (i = 0; i < 6; i++)
	//	ilbc_codec->enc.ste.ChoiceSTE[i] = 0;
	//iLBC_encode((unsigned char *)output->buf + output->size, 
	//	    ilbc_codec->enc_block, 
	//	    &ilbc_codec->enc, &ilbc_codec->enc.ste);
...
#else
	iLBCDecode(ilbc_codec->dec_block, (short *)input->buf, &ilbc_codec->dec, 1, 0, NULL);

	//for (i = 0; i < 6; i++)
	//	ilbc_codec->enc.ste.ChoiceSTE[i] = 0;
  //  iLBC_decode(ilbc_codec->dec_block, (unsigned char*) input->buf,
		//&ilbc_codec->dec, 1, &ilbc_codec->dec.ste);
				      ...
				      struct pjmedia_frame *output)
{
	...
	iLBCDecode(ilbc_codec->dec_block, NULL, &ilbc_codec->dec, 0, 0, NULL);

	//for (i = 0; i < 6; i++)
	//	ilbc_codec->enc.ste.ChoiceSTE[i] = 0;
 //   iLBC_decode(ilbc_codec->dec_block, NULL, &ilbc_codec->dec, 0, &ilbc_codec->dec.ste);
