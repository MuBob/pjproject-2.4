...
// 	UINT CStegSuit::STMR(void * pCarrier, char SecMsg);	
	...
	UINT Send (void * pSrc, int length, int type);
	UINT Receive (void * pDst, int maxlength, int type);
	UINT Embedding( void * pCarrier,UINT RTPheadlen, /*CAudioBase * pACIn*/char* pPcmIn, unsigned channel_pt);
	UINT Retransmission();
	...
	UINT Retriving(void *hdr, void * pCarrier,/*CAudioBase * pACOut*/char* pPcmOut, unsigned channel_pt);
...
// 	CStegLSB * m_pRTP;
	...
	unsigned m_channel_pt_receive;
	unsigned m_channel_pt_send;
